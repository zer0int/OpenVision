  ...
  def step_fn(step):
    ...
    if decay_type in ("linear", "polynomial"):
      ...
      if kw.get('min_lr'):
          ...
          lr = min_lr_ratio + (lr - min_lr_ratio) * 0.5 * (1. + jnp.cos(jnp.pi * progress))
      else:
...
def make(config, params, *, sched_kw):
  ...
  if config.get('lwd'):
      ...
      num_layer = decode_variant(config.model.variant)['depth']
      lwd_mults = [(".*encoderblock_"+str(i)+"/.*", config.lwd**(num_layer-i)) for i in range(num_layer)]
  ...
  return optax.chain(
      grad_clip_norm_tx,
      *opt_txs,
      *weight_decay_txs,  # exchange the order of learning rate
      *lr_mult_txs,
      *schedule_txs,
