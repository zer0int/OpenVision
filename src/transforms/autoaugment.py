...
def _randomly_negate_tensor(tensor):
  ...
  final_tensor = tf.cond(tf.less(tf.random.uniform([]), 0.5), lambda: tensor, lambda: -tensor)
  return final_tensor
...
def _enhance_increasing_level_to_arg(level):
    ...
    r = 1. + _randomly_negate_tensor(level)
    level = tf.cond(tf.less(0.1,  r), lambda: r,  lambda: 0.1 )
    #level = max(0.1, 1.0 + r)  # keep it >= 0.1
    return level,
...
def min_tensor(a, b):
    return tf.cond(tf.less(a, b), lambda: tf.cast(a,tf.uint8), lambda:tf.cast(b, dtype=tf.uint8))



def level_to_arg_increasing(hparams):
  return {
      'AutoContrast': lambda level: (),
      'Equalize': lambda level: (),
      'Invert': lambda level: (),
      'Rotate': _rotate_level_to_arg,
      'Posterize': lambda level: (4 - min_tensor(4, int((level/_MAX_LEVEL) * 4)),),
      'Solarize': lambda level: (256 - min_tensor(256, int((level/_MAX_LEVEL) * 256)),),
      'SolarizeAdd': lambda level: (min_tensor(128, int((level/_MAX_LEVEL) * 110)),),
...
def _parse_policy_info(name, prob, level, replace_value, augmentation_hparams, increase=True):
  """Return the function that corresponds to `name` and update `level` param."""
  func = NAME_TO_FUNC[name]
  if not increase:
      args = level_to_arg(augmentation_hparams)[name](level)
  else:
      args = level_to_arg_increasing(augmentation_hparams)[name](level)
...
def build_and_apply_nas_policy(policies, image,
                               augmentation_hparams, increase=True):
  ...
  for policy in policies:
    ...
    for policy_info in policy:
      ...
      tf_policy.append(_parse_policy_info(*policy_info, increase=increase))
...
def distort_image_with_randaugment(image, num_layers, magnitude, increase=False):
  ...
  """
  replace_value = [128] * 3
  tf.logging.info('Using BV RandAug with.', str(num_layers),str(magnitude), str(increase),)
  augmentation_hparams = HParams(
  ...
  available_ops = [
      ...
      'ShearX',
      'ShearY',
      'TranslateX', # same as relative trans in timm (because timm use a global constant)
      'TranslateY',
      #'Cutout', # no cutout when random erasing applied
     ]
  org_img = image
  for layer_num in range(num_layers):
    op_to_select = tf.random_uniform(
        [], maxval=len(available_ops), dtype=tf.int32)
   # random_magnitude = float(random.gauss(magnitude, 0.5)) # follow the timm
    ...
    with tf.name_scope('randaug_layer_{}'.format(layer_num)):
      for (i, op_name) in enumerate(available_ops):
        ...
        func, _, args = _parse_policy_info(op_name, prob, random_magnitude,
                                           replace_value, augmentation_hparams, increase=increase)
        image = tf.cond(
            tf.equal(i, op_to_select),
             lambda selected_func=func, selected_args=args: selected_func(
                 image, *selected_args),
           # lambda: _apply_func_with_prob(func, image,  args, 0.5),
            # pylint:enable=g-long-lambda
            lambda: image)
...
def check_zero_image(test_img, org_img):
    aug_image_np = test_img
    return tf.cond(tf.equal(tf.reduce_sum(aug_image_np), 0), lambda : org_img, lambda : test_img)


def distort_image_with_randaugment_timm(image, num_layers, magnitude, increase=False, num_zeros = 0):
  """Applies the RandAugment policy to `image`.
  RandAugment is from the paper https://arxiv.org/abs/1909.13719,
  Args:
    image: `Tensor` of shape [height, width, 3] representing an image.
  ...
  replace_value = [124, 116, 104] #* 3
  tf.logging.info('Using Timm RandAug with.'+str(num_layers)+'_'+str(magnitude)+'_'+str(increase),)
  augmentation_hparams = HParams(
      cutout_const=40, translate_const=100)
  available_ops = [
      'AutoContrast',
  ...
  for layer_num in range(num_layers):
    ...
    random_magnitude = tf.random.normal([], mean=magnitude, stddev=0.5) # follow the timm

    with tf.name_scope('randaug_layer_{}'.format(layer_num)):
        for (i, op_name) in enumerate(available_ops):
            aug_prob = tf.random_uniform([], minval=0., maxval=1., dtype=tf.float32)
            prob = tf.random_uniform([], minval=0.2, maxval=0.8, dtype=tf.float32)
            # prob = 0.5  # follow the timm
            func, _, args = _parse_policy_info(op_name, prob, random_magnitude, replace_value, augmentation_hparams, increase=increase)

            argument1 = lambda: tf.cond(tf.equal(i, op_to_select),
                                lambda selected_func=func, selected_args=args: selected_func(
                                    image, *selected_args),
                                lambda: image)
            argument2 = lambda: image
            image = tf.cond(tf.less(aug_prob, 0.5),
                        argument1,  argument2
                        )
       # np_img = np.array(image)
       # if np.equal(np_img, np.zeros_like(np_img)).all():
       #         num_zeros += 1
               # print(layer_num, op_name, args)
