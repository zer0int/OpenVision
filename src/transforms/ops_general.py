...
def get_copy(inkey1, inkey2, outkey, prob=0.5):
  ...
  def _copy(data):
    ...
    # and keep tensors as they are, since we never modify them in-place anyways.
    # The following achieves exactly that.
    def f1(): return jax.tree_map(lambda x: x, data[inkey1])
    def f2(): return jax.tree_map(lambda x: x, data[inkey2])
    cond = tf.math.less(tf.random.uniform(shape=[], minval=0., maxval=1.), prob)
    data[outkey] = tf.cond(cond, f1, f2)
