...
from transformers import FlaxCLIPTextModel, AutoTokenizer

from src.evaluators.proj.image_text import prompt_engineering
from src.transforms import ops_general  # pylint: disable=unused-import
from src.transforms import ops_image  # pylint: disable=unused-import
...
def prepare_datasets(img_dataset,
    ...
    def substitute_prompt(features):
        ...
        return {"label": features["label"], "texts": tf.strings.join(
            [parts[0], features["class_name"], parts[1]])}
    ...
    if isinstance(prompt_templates, dict):
        all_class_prompts = []
        all_class_ids = []
        for ids, key in enumerate(class_names):
            if key in prompt_templates.keys():
                all_class_prompts.extend(prompt_templates[key])
                all_class_ids.extend(tf.cast(tf.repeat([ids+class_name_offset], len(prompt_templates[key])), tf.int64))
            else:
                print(f'missing classes of {key}')
        ds_texts = tf.data.Dataset.from_tensor_slices({'label': all_class_ids, 'texts': all_class_prompts}).map(
...
def _split_and_batch(dataset_name, data_dir, class_names, batch_size, split,
    ...
    return (
        _with_infinite_padding(ds_images).batch(batch_size // jax.process_count()),
        _with_infinite_padding(ds_texts).batch(batch_size // jax.process_count()),
    )
...
class Evaluator:
    ...
    def __init__(self,
                 ...
                 *,
                 ...
                 tokenizer=None,
                 token_len=128, # for openclip tokenizer only
        ...
        dataset_overrides = {
            k: dict(v) for k,
            v in dict(dataset_overrides).items()}
        ...
        for dataset_name in dataset_names:
            ...
            class_names_dataset_name = overrides.pop(
                "class_names_dataset_name", dataset_name_)
    ...
    def _embed_texts(self, train_state, dataset_name):
        ...
        ds_b = input_pipeline.start_input_pipeline_eval(
            self.datasets[dataset_name]["texts"], mesh=self.mesh, data_sharding=self.data_sharding,
            tokenizer=self.tokenizer, context_length=self.token_len
        )
        for batch in ds_b:
    ...
    def evaluate(self,
        ...
        ztxt_p = reshard(ztxt_p, NamedSharding(self.mesh, P()))
        ...
        ds_b = input_pipeline.start_input_pipeline_eval(
            self.datasets[dataset_name]["images"], mesh=self.mesh, data_sharding=self.data_sharding)
        for batch in ds_b:
            ns.append(jax.device_get(self._count_p(batch["mask"])))
            if len(ns) >= self._async_delay and ns[-self._async_delay] == 0:
                break
