...
from src.evaluators.proj.image_text import image_text_retrieval
...
def prepare_datasets(dataset, *, pp_img, pp_txt, txt_name, offset=0, flikr=False):
  ...
  get_captions = functools.partial(get_captions, flikr=flikr)
...
class Evaluator:
  ...
  def __init__(self,
    ...
    self.ds_images, self.ds_texts = _split_and_batch(
        ...
        functools.partial(
            prepare_datasets, pp_img=pp_img, pp_txt=pp_txt, txt_name=txt_name, flikr=True if 'flickr30k' in data_dir else False),
        data_dir=data_dir)
  ...
  def _embed(self, name, train_state, ds, embed_fn, id_names):
     ...
     # ds_b = input_pipeline.start_input_pipeline_eval(ds, mesh=self.mesh, data_sharding=self.data_sharding)
      ds_b = input_pipeline.start_input_pipeline_eval(
          ds, mesh=self.mesh, data_sharding=self.data_sharding,
