...
import src.losses.common as losses
from src.losses import bidirectional_contrastive_loss
from src.datasets import input_pipeline
import src.datasets.core as ds_core
...
def _all_gather(z):
    ...
    return jax.tree.map(gather_flat, z)
...
def get_eval_fn(predict_fn, use_global_batch):
    ...
    def _eval_fn(params, images, labels, mask):
        ...
        # Temperature won't affect ranking for accuracy, but impacts loss
        # magnitude.
        losses, measurements = bidirectional_contrastive_loss(
