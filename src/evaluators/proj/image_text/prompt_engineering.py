...
import src.datasets.in1k_class_names  as imagenet_class_names
from src.evaluators.proj.image_text import prompt_engineering_constants
...
def get_prompt_templates(prompt_templates_name,
  ...
  if prompt_templates_name.endswith('json'):
      import json,os
      with open(os.path.join(os.getcwd(),'evaluators/proj/image_text', prompt_templates_name), 'r') as f:
          prompts_templates = json.load(f)
  else:
     prompts_templates = _PROMPT_TEMPLATES[prompt_templates_name]
  if canonicalize:
    if isinstance(prompts_templates, dict):
        new_templates = {}
