...
def from_config(config, predict_fns,
  ...
  for name, cfg in specs.items():
    ...
    module = importlib.import_module(f"src.evaluators.{module}")

    if mesh is not None and config is not None:
      cfg["mesh"] = mesh
      cfg['data_sharding'] = data_sharding
      cfg['train_state_sharding'] = train_state_sharding

    if tokenizer is not None:
      cfg["tokenizer"] = tokenizer
      cfg['token_len'] = config.openclip_tokenizer.token_len
