...
# [LAB]use python3.10 instead of python3 or cannot found packages
#cd /usr/lib/python3/dist-packages && cp  apt_pkg.cpython-38-x86_64-linux-gnu.so
sudo add-apt-repository -y ppa:deadsnakes/ppa #install python3.10 for google-jetstram
sudo apt install -y  python3.10-venv
python3.10 -m venv openvision_env

. $HOME/openvision/openvision_env/bin/activate
